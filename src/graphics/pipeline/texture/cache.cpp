...
void TextureCache::RequestTextures(uint32_t used_texture_mask) {
  ...
  while (rex::bit_scan_forward(textures_remaining, &index)) {
    ...
    binding.integer_scale_bits = GetIntegerScaleBits(fetch.format, fetch.num_format,
                                                     fetch.swizzle, binding.swizzled_signs);
...
// Ported from xenia-canary d11950528 + 2ddc5ef73 + 6a4545208.
