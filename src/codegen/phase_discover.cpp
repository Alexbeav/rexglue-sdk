...
void discoverFunction(CodegenContext& ctx, uint32_t funcAddr,
  ...
  if (node->authority() == FunctionAuthority::CONFIG) {
    ...
    auto cfgIt = ctx.Config().functions.find(funcAddr);
    if (cfgIt != ctx.Config().functions.end() && cfgIt->second.isChunk()) {
      extentStart = cfgIt->second.parent;
      if (auto* parent = graph.getFunction(extentStart)) {
        pdataSize = parent->size();
      }
